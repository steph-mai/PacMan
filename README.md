...
- **Ghost State Machine:** The ghosts use a "State Machine" combined with a global phase cycle to govern their behavior. They periodically alternate between **SCATTER** (patrol mode, where each ghost moves randomly) and **CHASING** (hunting the player). They can also dynamically switch to **RUNNING_AWAY** (scared mode when a Super Pacgum is eaten) and **DEAD** (returning to their spawn point).
